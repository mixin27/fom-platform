...
export async function getDocument(slug: string) {
  try {
    ...
    const fallbackRoute = PageRoutes.find((route) => route.href === `/${slug}`)
    const frontmatter = {
      title: parsedMdx.frontmatter.title || fallbackRoute?.title || 'Untitled',
      description: parsedMdx.frontmatter.description || '',
      keywords: parsedMdx.frontmatter.keywords || [],
    }
...
export async function getTable(
  ...
  while ((match = headingsRegex.exec(rawMdx)) !== null) {
    const [, headingDepth = '', headingText = ''] = match
    const level = headingDepth.length
